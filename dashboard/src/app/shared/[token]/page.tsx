...
export default function SharedViewPage() {
  ...
  const tColor = temp != null && th ? (temp >= th.critical ? "#ef4444" : temp >= th.safe_max ? "#f59e0b" : "#10b981") : "#94a3b8";
    ...
    <div className="min-h-screen p-6 flex flex-col items-center">
      <div className="w-full max-w-3xl fade-in">
        <header className="flex items-center justify-between border-b border-[var(--border)] pb-4 mb-6">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-[var(--text-dim)] tracking-wider">SHARED VIEW — READ ONLY</p>
            </div>
          </div>
        ...
        {error ? (
          ...
          <div className="text-center py-20 text-[var(--text-muted)]">Loading status…</div>
        ) : (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Card>
                ...
                {th && <p className="text-[11px] text-[var(--text-dim)] mt-2">Safe ≤ {th.safe_max}°C · Critical {th.critical}°C</p>}
              </Card>
              <Card>
                ...
                <div className="metric text-4xl" style={{ color: hColor }}>
                  ...
                  <span className="text-base text-[var(--text-dim)] font-normal"> / 100</span>
                </div>
                <p className="text-[11px] text-[var(--text-dim)] mt-2">Trend: {status.health_trend || "—"}</p>
              </Card>
            </div>
            ...
            <Card>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="metric text-2xl uppercase text-[var(--text)] mt-1">{status.action || "—"}</p>
                </div>
                <div className="text-right">
                  <p className="eyebrow">Fault Signature</p>
                  <p className="text-sm text-[var(--text)] mt-1">{status.fault_type || "Normal"}</p>
                </div>
              </div>
            </Card>
